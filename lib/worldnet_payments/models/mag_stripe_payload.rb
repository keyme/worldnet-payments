...
module WorldnetPayments
  class MagStripePayload < Payload
    # This field gives clients the ability to reprocess offline transactions that were not approved initially in a different entry mode.<br />For instance, `MAG_STRIPE` transactions are allowed to be downgraded to `KEYED`.
    attr_accessor :downgrade_to

    # The cardholder's name as it appears on the card.
    attr_accessor :cardholder_name

    # Cardholder's signature in the format described in the [Special Fields and Parameters](https://docs.worldnettps.com/doku.php?id=developer:api_specification:special_fields_and_parameters#the_signature_field_format) section.
    attr_accessor :cardholder_signature

    attr_accessor :card_details

    attr_accessor :pin_details

    attr_accessor :ebt_details

    # Additional TLV (Tag-Length-Value) data which is sometimes required for contactless transactions.
    attr_accessor :additional_tlv_data
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::MagStripePayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::MagStripePayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'downgrade_to')
        self.downgrade_to = attributes[:'downgrade_to']
      end
      ...
      if attributes.key?(:'cardholder_signature')
        self.cardholder_signature = attributes[:'cardholder_signature']
      end
      ...
      if attributes.key?(:'pin_details')
        self.pin_details = attributes[:'pin_details']
      end

      if attributes.key?(:'ebt_details')
        self.ebt_details = attributes[:'ebt_details']
      end

      if attributes.key?(:'additional_tlv_data')
        self.additional_tlv_data = attributes[:'additional_tlv_data']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@cardholder_name.nil? && @cardholder_name.to_s.length > 50
        invalid_properties.push('invalid value for "cardholder_name", the character length must be smaller than or equal to 50.')
      end

      if !@cardholder_name.nil? && @cardholder_name.to_s.length < 0
        invalid_properties.push('invalid value for "cardholder_name", the character length must be great than or equal to 0.')
      end

      if @card_details.nil?
        invalid_properties.push('invalid value for "card_details", card_details cannot be nil.')
      end
    ...
    def valid?
      downgrade_to_validator = EnumAttributeValidator.new('String', ["MAG_STRIPE", "KEYED"])
      return false unless downgrade_to_validator.valid?(@downgrade_to)
      return false if !@cardholder_name.nil? && @cardholder_name.to_s.length > 50
      return false if !@cardholder_name.nil? && @cardholder_name.to_s.length < 0
      return false if @card_details.nil?
      true && super
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] downgrade_to Object to be assigned
    def downgrade_to=(downgrade_to)
      validator = EnumAttributeValidator.new('String', ["MAG_STRIPE", "KEYED"])
      unless validator.valid?(downgrade_to)
        fail ArgumentError, "invalid value for \"downgrade_to\", must be one of #{validator.allowable_values}."
      end
    ...
    def cardholder_name=(cardholder_name)
      if !cardholder_name.nil? && cardholder_name.to_s.length > 50
        fail ArgumentError, 'invalid value for "cardholder_name", the character length must be smaller than or equal to 50.'
      end

      if !cardholder_name.nil? && cardholder_name.to_s.length < 0
        fail ArgumentError, 'invalid value for "cardholder_name", the character length must be great than or equal to 0.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          downgrade_to == o.downgrade_to &&
          cardholder_name == o.cardholder_name &&
          cardholder_signature == o.cardholder_signature &&
          card_details == o.card_details &&
          pin_details == o.pin_details &&
          ebt_details == o.ebt_details &&
          additional_tlv_data == o.additional_tlv_data && super(o)
    end
    ...
    def hash
      [downgrade_to, cardholder_name, cardholder_signature, card_details, pin_details, ebt_details, additional_tlv_data].hash
    end
