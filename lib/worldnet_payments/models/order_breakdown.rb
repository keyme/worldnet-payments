...
module WorldnetPayments
  class OrderBreakdown
    # The amount excluding taxes, tip and cashback amounts.
    attr_accessor :subtotal_amount

    # Cashback amount.
    attr_accessor :cashback_amount

    # List of taxes to be applied to the order.
    attr_accessor :taxes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::OrderBreakdown` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::OrderBreakdown`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cashback_amount')
        self.cashback_amount = attributes[:'cashback_amount']
      end
    ...
    def list_invalid_properties
      ...
      if @subtotal_amount.nil?
        invalid_properties.push('invalid value for "subtotal_amount", subtotal_amount cannot be nil.')
      end

      if @subtotal_amount <= 0
        invalid_properties.push('invalid value for "subtotal_amount", must be greater than 0.')
      end

      if !@cashback_amount.nil? && @cashback_amount <= 0
        invalid_properties.push('invalid value for "cashback_amount", must be greater than 0.')
      end
    ...
    def valid?
      return false if @subtotal_amount.nil?
      return false if @subtotal_amount <= 0
      return false if !@cashback_amount.nil? && @cashback_amount <= 0
      true
    end
    ...
    def subtotal_amount=(subtotal_amount)
      if subtotal_amount.nil?
        fail ArgumentError, 'subtotal_amount cannot be nil'
      end

      if subtotal_amount <= 0
        fail ArgumentError, 'invalid value for "subtotal_amount", must be greater than 0.'
      end
    ...
    def cashback_amount=(cashback_amount)
      if !cashback_amount.nil? && cashback_amount <= 0
        fail ArgumentError, 'invalid value for "cashback_amount", must be greater than 0.'
      end
    ...
    def hash
      [subtotal_amount, cashback_amount, taxes, tip, surcharge].hash
    end
