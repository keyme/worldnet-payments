...
module WorldnetPayments
  class StoreCredentialsRequest
    ...
    # Unique merchant reference that identifies the stored credentials on both merchant's and gateway's sides.<br />If not provided, we will assign an auto-generated value to the credentials and returns it in the response.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::StoreCredentialsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::StoreCredentialsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@merchant_reference.nil? && @merchant_reference.to_s.length > 48
        invalid_properties.push('invalid value for "merchant_reference", the character length must be smaller than or equal to 48.')
      end

      if !@merchant_reference.nil? && @merchant_reference.to_s.length < 0
        invalid_properties.push('invalid value for "merchant_reference", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@merchant_reference.nil? && @merchant_reference.to_s.length > 48
      return false if !@merchant_reference.nil? && @merchant_reference.to_s.length < 0
    ...
    def merchant_reference=(merchant_reference)
      ...
      if !merchant_reference.nil? && merchant_reference.to_s.length < 0
        fail ArgumentError, 'invalid value for "merchant_reference", the character length must be great than or equal to 0.'
      end
    ...
    def hash
      [terminal, operator, merchant_reference, ip_address, customer, customer_account, additional_data_fields].hash
    end
