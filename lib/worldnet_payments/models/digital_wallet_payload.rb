...
module WorldnetPayments
  class DigitalWalletPayload < Payload
    # The wallet application provider.
    attr_accessor :service_provider

    # The cardholder's name as it appears on the card.
    attr_accessor :cardholder_name

    # The encrypted data generated by the digital wallet application.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::DigitalWalletPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::DigitalWalletPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @encrypted_data.to_s.length > 20480
        invalid_properties.push('invalid value for "encrypted_data", the character length must be smaller than or equal to 20480.')
      end

      if @encrypted_data.to_s.length < 128
        invalid_properties.push('invalid value for "encrypted_data", the character length must be great than or equal to 128.')
      end
    ...
    def valid?
      ...
      service_provider_validator = EnumAttributeValidator.new('String', ["GOOGLE", "APPLE"])
      return false unless service_provider_validator.valid?(@service_provider)
      ...
      return false if @encrypted_data.to_s.length > 20480
      return false if @encrypted_data.to_s.length < 128
      true && super
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] service_provider Object to be assigned
    def service_provider=(service_provider)
      validator = EnumAttributeValidator.new('String', ["GOOGLE", "APPLE"])
      unless validator.valid?(service_provider)
        fail ArgumentError, "invalid value for \"service_provider\", must be one of #{validator.allowable_values}."
      end
    ...
    def encrypted_data=(encrypted_data)
      ...
      if encrypted_data.to_s.length > 20480
        fail ArgumentError, 'invalid value for "encrypted_data", the character length must be smaller than or equal to 20480.'
      end

      if encrypted_data.to_s.length < 128
        fail ArgumentError, 'invalid value for "encrypted_data", the character length must be great than or equal to 128.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          encrypted_data == o.encrypted_data && super(o)
    end
    ...
    def hash
      [service_provider, cardholder_name, encrypted_data].hash
    end
