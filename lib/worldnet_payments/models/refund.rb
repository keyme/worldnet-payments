...
module WorldnetPayments
  class Refund
    # Unique reference number assigned by the gateway that identifies the transaction on both platforms.  **Note:** Clients must be able to store this value in order to eventually perform follow up operation on existing transactions.
    attr_accessor :unique_reference

    # The terminal number assigned by the gateway.
    attr_accessor :terminal

    # The operator who initiated the transaction.
    attr_accessor :operator

    # A unique identifier for the order assigned by the merchant.
    attr_accessor :order_id

    # The reason why the transaction is being refunded.
    ...
    # List of custom fields representing the additional information sent by the merchant and stored along with the transaction.
    attr_accessor :additional_data_fields

    # List of tags returned for EMV transactions.
    attr_accessor :emv_tags

    # The customer and merchant receipt copies.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Refund` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Refund`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'unique_reference')
        self.unique_reference = attributes[:'unique_reference']
      end
      ...
      if attributes.key?(:'customer_account')
        self.customer_account = attributes[:'customer_account']
      end
      ...
      if attributes.key?(:'emv_tags')
        if (value = attributes[:'emv_tags']).is_a?(Array)
          self.emv_tags = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @unique_reference.nil?
        invalid_properties.push('invalid value for "unique_reference", unique_reference cannot be nil.')
      end

      if @unique_reference.to_s.length > 10
        invalid_properties.push('invalid value for "unique_reference", the character length must be smaller than or equal to 10.')
      end

      if @unique_reference.to_s.length < 10
        invalid_properties.push('invalid value for "unique_reference", the character length must be great than or equal to 10.')
      end
      ...
      if !@operator.nil? && @operator.to_s.length > 50
        invalid_properties.push('invalid value for "operator", the character length must be smaller than or equal to 50.')
      end

      if !@operator.nil? && @operator.to_s.length < 0
        invalid_properties.push('invalid value for "operator", the character length must be great than or equal to 0.')
      end
      ...
      if @order_id.to_s.length > 24
        invalid_properties.push('invalid value for "order_id", the character length must be smaller than or equal to 24.')
      end
      ...
      if @refund_reason.to_s.length > 100
        invalid_properties.push('invalid value for "refund_reason", the character length must be smaller than or equal to 100.')
      end

      if @refund_reason.to_s.length < 1
        invalid_properties.push('invalid value for "refund_reason", the character length must be great than or equal to 1.')
      end

      if @customer_account.nil?
        invalid_properties.push('invalid value for "customer_account", customer_account cannot be nil.')
      end
    ...
    def valid?
      return false if @unique_reference.nil?
      return false if @unique_reference.to_s.length > 10
      return false if @unique_reference.to_s.length < 10
      return false if @terminal.nil?
      return false if @terminal.to_s.length > 50
      return false if @terminal.to_s.length < 1
      return false if !@operator.nil? && @operator.to_s.length > 50
      return false if !@operator.nil? && @operator.to_s.length < 0
      return false if @order_id.nil?
      return false if @order_id.to_s.length > 24
      return false if @order_id.to_s.length < 1
      return false if @refund_reason.nil?
      return false if @refund_reason.to_s.length > 100
      return false if @refund_reason.to_s.length < 1
      return false if @customer_account.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] unique_reference Value to be assigned
    def unique_reference=(unique_reference)
      if unique_reference.nil?
        fail ArgumentError, 'unique_reference cannot be nil'
      end

      if unique_reference.to_s.length > 10
        fail ArgumentError, 'invalid value for "unique_reference", the character length must be smaller than or equal to 10.'
      end

      if unique_reference.to_s.length < 10
        fail ArgumentError, 'invalid value for "unique_reference", the character length must be great than or equal to 10.'
      end
    ...
    def operator=(operator)
      if !operator.nil? && operator.to_s.length > 50
        fail ArgumentError, 'invalid value for "operator", the character length must be smaller than or equal to 50.'
      end

      if !operator.nil? && operator.to_s.length < 0
        fail ArgumentError, 'invalid value for "operator", the character length must be great than or equal to 0.'
      end
    ...
    def order_id=(order_id)
      ...
      if order_id.to_s.length > 24
        fail ArgumentError, 'invalid value for "order_id", the character length must be smaller than or equal to 24.'
      end
    ...
    def refund_reason=(refund_reason)
      ...
      if refund_reason.to_s.length > 100
        fail ArgumentError, 'invalid value for "refund_reason", the character length must be smaller than or equal to 100.'
      end

      if refund_reason.to_s.length < 1
        fail ArgumentError, 'invalid value for "refund_reason", the character length must be great than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] additional_data_fields Value to be assigned
    def additional_data_fields=(additional_data_fields)
      @additional_data_fields = additional_data_fields
    end

    # Custom attribute writer method with validation
    # @param [Object] emv_tags Value to be assigned
    def emv_tags=(emv_tags)
      @emv_tags = emv_tags
    end

    # Custom attribute writer method with validation
    # @param [Object] receipts Value to be assigned
    ...
    def hash
      [unique_reference, terminal, operator, order_id, refund_reason, customer_account, security_check, transaction_result, additional_data_fields, emv_tags, receipts, links].hash
    end
