  ...
  class SurchargeSettings
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::SurchargeSettings` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::SurchargeSettings`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@percentage.nil? && @percentage > 4.0
        invalid_properties.push('invalid value for "percentage", must be smaller than or equal to 4.0.')
      end

      if !@percentage.nil? && @percentage < 0.1
        invalid_properties.push('invalid value for "percentage", must be greater than or equal to 0.1.')
      end
    ...
    def valid?
      return false if !@percentage.nil? && @percentage > 4.0
      return false if !@percentage.nil? && @percentage < 0.1
      true
    end
    ...
    def percentage=(percentage)
      if !percentage.nil? && percentage > 4.0
        fail ArgumentError, 'invalid value for "percentage", must be smaller than or equal to 4.0.'
      end

      if !percentage.nil? && percentage < 0.1
        fail ArgumentError, 'invalid value for "percentage", must be greater than or equal to 0.1.'
      end
