...
module WorldnetPayments
  ...
  class TerminalCompact
    ...
    # Terminal's preferred time-zone.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::TerminalCompact` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::TerminalCompact`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@payment_processor.nil? && @payment_processor.to_s.length > 128
        invalid_properties.push('invalid value for "payment_processor", the character length must be smaller than or equal to 128.')
      end

      if !@payment_processor.nil? && @payment_processor.to_s.length < 1
        invalid_properties.push('invalid value for "payment_processor", the character length must be great than or equal to 1.')
      end

      if !@terminal_number.nil? && @terminal_number.to_s.length > 50
        invalid_properties.push('invalid value for "terminal_number", the character length must be smaller than or equal to 50.')
      end

      if !@terminal_number.nil? && @terminal_number.to_s.length < 1
    ...
    def valid?
      return false if !@payment_processor.nil? && @payment_processor.to_s.length > 128
      return false if !@payment_processor.nil? && @payment_processor.to_s.length < 1
      return false if !@terminal_number.nil? && @terminal_number.to_s.length > 50
      return false if !@terminal_number.nil? && @terminal_number.to_s.length < 1
    ...
    def payment_processor=(payment_processor)
      ...
      if !payment_processor.nil? && payment_processor.to_s.length < 1
        fail ArgumentError, 'invalid value for "payment_processor", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [payment_processor, terminal_number, display_name, base_currency, time_zone, links].hash
    end
