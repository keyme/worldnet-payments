...
module WorldnetPayments
  class PlainTextMagStripeDataFormat < MagStripeDataFormat
    ...
    def self.openapi_all_of
      [
      :'MagStripeDataFormat',
      :'PlainTextMagStripeDataFormatAllOf'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::PlainTextMagStripeDataFormat` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::PlainTextMagStripeDataFormat`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @track_data.nil?
        invalid_properties.push('invalid value for "track_data", track_data cannot be nil.')
      end

      if @track_data.to_s.length > 256
        invalid_properties.push('invalid value for "track_data", the character length must be smaller than or equal to 256.')
      end

      if @track_data.to_s.length < 16
        invalid_properties.push('invalid value for "track_data", the character length must be great than or equal to 16.')
      end
    ...
    def valid?
      ...
      return false if @track_data.nil?
      return false if @track_data.to_s.length > 256
      return false if @track_data.to_s.length < 16
      true && super
    end

    # Custom attribute writer method with validation
    # @param [Object] track_data Value to be assigned
    def track_data=(track_data)
      if track_data.nil?
        fail ArgumentError, 'track_data cannot be nil'
      end

      if track_data.to_s.length > 256
        fail ArgumentError, 'invalid value for "track_data", the character length must be smaller than or equal to 256.'
      end

      if track_data.to_s.length < 16
        fail ArgumentError, 'invalid value for "track_data", the character length must be great than or equal to 16.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          device == o.device &&
          track_data == o.track_data && super(o)
    end
