...
module WorldnetPayments
  ...
  class TransactionCompact
    ...
    # The card or account holder's name.
    attr_accessor :customer_name

    # The masked primary account number.
    attr_accessor :masked_pan

    # The type of the generated transaction.
    attr_accessor :type

    # Our platform maps result codes sent by different payment processors into a smaller set of codes as shown below. The original result code may be available in the `processorResultCode` field.  - **A**: Approved / Authorized. - **D**: Declined / Not Authorized. - **E**: Accepted for later processing, but result currently unknown. - **P**: Only a portion of the original amount requested was authorized. - **R**: Issuer has declined the transaction and indicated that the customer should contact their bank. - **C**: Issuer has declined the transaction and requested that the card be retained as it may have been reported as lost or stolen.
    attr_accessor :result_code

    # The current status of the generated transaction.
    attr_accessor :status

    # The currency of the transaction. A 3-letter code as per the [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217#Active_codes) standard.
    attr_accessor :currency

    # The amount authorized by the payment processor.  **Note:** For partial authorizations, this amount will be lower than the actual amount sent in the request.<br />**Note:** This amount will be negative for refund transactions to represent the return of settled funds back to the customer's account.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::TransactionCompact` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::TransactionCompact`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@customer_name.nil? && @customer_name.to_s.length > 50
        invalid_properties.push('invalid value for "customer_name", the character length must be smaller than or equal to 50.')
      end

      if !@customer_name.nil? && @customer_name.to_s.length < 0
        invalid_properties.push('invalid value for "customer_name", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@customer_name.nil? && @customer_name.to_s.length > 50
      return false if !@customer_name.nil? && @customer_name.to_s.length < 0
      result_code_validator = EnumAttributeValidator.new('String', ["A", "D", "E", "P", "R", "C"])
      return false unless result_code_validator.valid?(@result_code)
      status_validator = EnumAttributeValidator.new('String', ["READY", "PENDING", "VOID", "DECLINED", "COMPLETE", "REFERRAL", "PICKUP", "REVERSAL", "SENT", "ADMIN", "EXPIRED", "ACCEPTED", "REVIEW", "OTHER"])
    ...
    def customer_name=(customer_name)
      if !customer_name.nil? && customer_name.to_s.length > 50
        fail ArgumentError, 'invalid value for "customer_name", the character length must be smaller than or equal to 50.'
      end

      if !customer_name.nil? && customer_name.to_s.length < 0
        fail ArgumentError, 'invalid value for "customer_name", the character length must be great than or equal to 0.'
      end
    ...
    def result_code=(result_code)
      validator = EnumAttributeValidator.new('String', ["A", "D", "E", "P", "R", "C"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["READY", "PENDING", "VOID", "DECLINED", "COMPLETE", "REFERRAL", "PICKUP", "REVERSAL", "SENT", "ADMIN", "EXPIRED", "ACCEPTED", "REVIEW", "OTHER"])
    ...
    def hash
      [unique_reference, terminal, operator, order_id, date_time, description, customer_name, masked_pan, type, result_code, status, currency, amount, links].hash
    end
