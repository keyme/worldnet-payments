...
module WorldnetPayments
  # Use this field to indicate that the surcharge fee configured in your account has been applied to the transaction's total amount.
  class Surcharge
    # Indicates whether or not a surcharge fee is applied to the total amount.
    attr_accessor :applied

    # Surcharge amount. This field is only available in the response and it'll be populated whenever a surcharge fee is applied to the transaction.
    attr_accessor :amount

    # Surcharge percentage. This field is only available in the response and it'll be populated whenever a surcharge fee is applied to the transaction.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Surcharge` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Surcharge`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [applied, amount, percentage].hash
    end
