...
module WorldnetPayments
  ...
  class ProcessingRule
    # Indicates whether the rule should be evaluated.
    attr_accessor :enable

    # The conditions that trigger the action, if met.
    ...
    def self.openapi_types
      {
        :'enable' => :'Boolean',
        :'conditions' => :'Array<ProcessingRulePredicate>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::ProcessingRule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::ProcessingRule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @_then.nil?
        invalid_properties.push('invalid value for "_then", _then cannot be nil.')
      end
