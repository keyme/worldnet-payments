...
module WorldnetPayments
  # The customer's account details used to authorize the transaction.
  class CustomerAccount
    ...
    # The method used to capture the customer's account details.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::CustomerAccount` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::CustomerAccount`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@cardholder_name.nil? && @cardholder_name.to_s.length < 1
        invalid_properties.push('invalid value for "cardholder_name", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@cardholder_name.nil? && @cardholder_name.to_s.length < 1
    ...
    def cardholder_name=(cardholder_name)
      ...
      if !cardholder_name.nil? && cardholder_name.to_s.length < 1
        fail ArgumentError, 'invalid value for "cardholder_name", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [card_type, cardholder_name, masked_pan, expiry_date, entry_method].hash
    end
