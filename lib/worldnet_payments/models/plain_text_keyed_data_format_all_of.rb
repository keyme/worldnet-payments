...
module WorldnetPayments
  class PlainTextKeyedDataFormatAllOf
    attr_accessor :device

    # The card number, as a string without any separators.
    attr_accessor :card_number

    # The card's expiration date in the format `MMYY`. This field is mandatory in most cases except when performing electronic voucher transactions.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::PlainTextKeyedDataFormatAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::PlainTextKeyedDataFormatAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [device, card_number, expiry_date, cvv, issue_number].hash
    end
