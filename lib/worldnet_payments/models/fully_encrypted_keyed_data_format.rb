...
module WorldnetPayments
  class FullyEncryptedKeyedDataFormat < KeyedDataFormat
    attr_accessor :device

    # The full card data encrypted by the device.
    ...
    def self.openapi_all_of
      [
      :'FullyEncryptedKeyedDataFormatAllOf',
      :'KeyedDataFormat'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::FullyEncryptedKeyedDataFormat` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::FullyEncryptedKeyedDataFormat`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          first_digit_of_pan == o.first_digit_of_pan && super(o)
    end
    ...
    def hash
      [device, encrypted_data, first_digit_of_pan].hash
    end
