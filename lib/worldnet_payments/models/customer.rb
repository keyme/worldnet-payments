...
module WorldnetPayments
  ...
  class Customer
    ...
    # The customer's contact phone number.<br />If **SMS Cardholder Receipts** feature is enabled in the terminal, We will use this number to automatically send the receipts of eventual transactions.
    attr_accessor :phone

    # The customer's contact email address.<br />If **Email Cardholder Receipt** feature is enabled in the terminal, We will use this address to automatically send the receipts of eventual transactions.
    attr_accessor :email

    # The customer's preferred notification language.<br />The two-letter language code defined by the [ISO 639-1 alpha-2](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes) standard.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Customer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Customer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@notification_language.nil? && @notification_language.to_s.length > 2
        invalid_properties.push('invalid value for "notification_language", the character length must be smaller than or equal to 2.')
      end

      if !@notification_language.nil? && @notification_language.to_s.length < 2
        invalid_properties.push('invalid value for "notification_language", the character length must be great than or equal to 2.')
      end
    ...
    def valid?
      ...
      notification_language_validator = EnumAttributeValidator.new('String', ["en", "fr"])
      return false unless notification_language_validator.valid?(@notification_language)
      return false if !@notification_language.nil? && @notification_language.to_s.length > 2
      return false if !@notification_language.nil? && @notification_language.to_s.length < 2
      true
    end
    ...
    # @param [Object] notification_language Object to be assigned
    def notification_language=(notification_language)
      validator = EnumAttributeValidator.new('String', ["en", "fr"])
      unless validator.valid?(notification_language)
        fail ArgumentError, "invalid value for \"notification_language\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [name, phone, email, notification_language, billing_address, shipping_address].hash
    end
