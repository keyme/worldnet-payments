...
module WorldnetPayments
  class RefundRequest
    ...
    # The operator who initiated the transaction. If not sent in the request, this field will be automatically populated with the API Key alias.
    ...
    # The reason why the refund is being performed.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::RefundRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::RefundRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @refund_amount <= 0
        invalid_properties.push('invalid value for "refund_amount", must be greater than 0.')
      end
    ...
    def refund_amount=(refund_amount)
      ...
      if refund_amount <= 0
        fail ArgumentError, 'invalid value for "refund_amount", must be greater than 0.'
      end
    ...
    def hash
      [channel, terminal, operator, order_id, currency, refund_amount, refund_reason, customer, customer_account, ip_address, additional_data_fields].hash
    end
