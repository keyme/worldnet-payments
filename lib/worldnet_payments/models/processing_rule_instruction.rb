...
module WorldnetPayments
  # It describes the action that should be executed.
  class ProcessingRuleInstruction
    # The action to be performed when the conditions are met.
    attr_accessor :action

    # The number of the terminal which is the target of the action.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::ProcessingRuleInstruction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::ProcessingRuleInstruction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      action_validator = EnumAttributeValidator.new('String', ["ROUTE_TO_TERMINAL"])
    ...
    def action=(action)
      validator = EnumAttributeValidator.new('String', ["ROUTE_TO_TERMINAL"])
