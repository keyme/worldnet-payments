...
module WorldnetPayments
  ...
  class DukptPinDetails
    # The PIN encrypted using [DUKPT](https://en.wikipedia.org/wiki/Derived_unique_key_per_transaction) Scheme.
    attr_accessor :pin

    # The key serial number.
    attr_accessor :pin_ksn
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::DukptPinDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::DukptPinDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pin_ksn')
        self.pin_ksn = attributes[:'pin_ksn']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @pin.to_s.length > 2147483647
        invalid_properties.push('invalid value for "pin", the character length must be smaller than or equal to 2147483647.')
      end
      ...
      if @pin_ksn.nil?
        invalid_properties.push('invalid value for "pin_ksn", pin_ksn cannot be nil.')
      end

      if @pin_ksn.to_s.length > 20
        invalid_properties.push('invalid value for "pin_ksn", the character length must be smaller than or equal to 20.')
      end

      if @pin_ksn.to_s.length < 20
        invalid_properties.push('invalid value for "pin_ksn", the character length must be great than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @pin.to_s.length > 2147483647
      return false if @pin.to_s.length < 1
      return false if @pin_ksn.nil?
      return false if @pin_ksn.to_s.length > 20
      return false if @pin_ksn.to_s.length < 20
      true
    end
    ...
    def pin=(pin)
      ...
      if pin.to_s.length > 2147483647
        fail ArgumentError, 'invalid value for "pin", the character length must be smaller than or equal to 2147483647.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] pin_ksn Value to be assigned
    def pin_ksn=(pin_ksn)
      if pin_ksn.nil?
        fail ArgumentError, 'pin_ksn cannot be nil'
      end

      if pin_ksn.to_s.length > 20
        fail ArgumentError, 'invalid value for "pin_ksn", the character length must be smaller than or equal to 20.'
      end

      if pin_ksn.to_s.length < 20
        fail ArgumentError, 'invalid value for "pin_ksn", the character length must be great than or equal to 20.'
      end
