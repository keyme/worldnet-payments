...
module WorldnetPayments
  class Balance
    ...
    # List of balance results per benefit category.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Balance` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Balance`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'account_balances')
        if (value = attributes[:'account_balances']).is_a?(Array)
          self.account_balances = value
        end
      end
    end
    ...
    def hash
      [terminal, operator, transaction_result, account_balances].hash
    end
