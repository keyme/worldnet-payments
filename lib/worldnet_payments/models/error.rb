...
module WorldnetPayments
  class Error
    # Clients should use this field when communicating to our support team. It allows us to easily identify the request and consequently the reason behind its failure.
    attr_accessor :debug_identifier
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Error` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Error`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'debug_identifier')
        self.debug_identifier = attributes[:'debug_identifier']
      end
    ...
    def list_invalid_properties
      ...
      if @debug_identifier.nil?
        invalid_properties.push('invalid value for "debug_identifier", debug_identifier cannot be nil.')
      end
    ...
    def valid?
      return false if @debug_identifier.nil?
    ...
    def ==(o)
      ...
      self.class == o.class &&
          debug_identifier == o.debug_identifier &&
          details == o.details
    end
