...
module WorldnetPayments
  class CardSecurityCodeVerification
    # Indicates whether CVV check is available for the terminal.
    attr_accessor :enable

    # Indicates that all CVV codes listed in `cvvDeclineCodes` property will lead to a transaction decline.This is sometimes desirable as not all card issuers will decline a transaction that has failed a CVV check.
    attr_accessor :auto_decline_on_failure

    # List of CVV result codes that will lead to a transaction decline when `autoDeclineTransactionsOnCvvFailures` is enabled.
    attr_accessor :decline_codes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::CardSecurityCodeVerification` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::CardSecurityCodeVerification`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'decline_codes')
        if (value = attributes[:'decline_codes']).is_a?(Array)
          self.decline_codes = value
        end
      end
    end
    ...
    def hash
      [enable, auto_decline_on_failure, decline_codes].hash
    end
