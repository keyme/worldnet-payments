...
module WorldnetPayments
  class ErrorDetail
    # A code assigned by the gateway to each type of failure.
    attr_accessor :error_code

    # A brief description of the given error.
    attr_accessor :error_message

    # A link to an external documentation that fully explains the issue.
    attr_accessor :about
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::ErrorDetail` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::ErrorDetail`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [error_code, error_message, about, source].hash
    end
