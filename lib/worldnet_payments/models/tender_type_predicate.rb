...
module WorldnetPayments
  class TenderTypePredicate < ProcessingRulePredicate
    # The comparison operator.
    attr_accessor :is

    # The required value for the condition to be met.
    attr_accessor :value
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::TenderTypePredicate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::TenderTypePredicate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      is_validator = EnumAttributeValidator.new('String', ["EQUALS"])
      return false unless is_validator.valid?(@is)
      return false if @value.nil?
      value_validator = EnumAttributeValidator.new('String', ["CREDIT_DEBIT", "EBT"])
    ...
    def is=(is)
      ...
      unless validator.valid?(is)
        fail ArgumentError, "invalid value for \"is\", must be one of #{validator.allowable_values}."
      end
      @is = is
    end
    ...
    def value=(value)
      validator = EnumAttributeValidator.new('String', ["CREDIT_DEBIT", "EBT"])
