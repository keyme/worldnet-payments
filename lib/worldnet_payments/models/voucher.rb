...
module WorldnetPayments
  # Allows for processing EBT transactions using a manual voucher. This option is only available for EBT food stamp benefit accounts.
  class Voucher
    # The authorization code issued by a customer service representative. This code is usually acquired via a phone call during which it is ensured that there are enough funds in the cardholder’s EBT account for the transaction.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Voucher` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Voucher`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @approval_code.nil?
        invalid_properties.push('invalid value for "approval_code", approval_code cannot be nil.')
      end

      if @approval_code.to_s.length > 6
        invalid_properties.push('invalid value for "approval_code", the character length must be smaller than or equal to 6.')
      end

      if @approval_code.to_s.length < 6
        invalid_properties.push('invalid value for "approval_code", the character length must be great than or equal to 6.')
      end
      ...
      if @serial_number.to_s.length > 15
        invalid_properties.push('invalid value for "serial_number", the character length must be smaller than or equal to 15.')
      end

      if @serial_number.to_s.length < 7
        invalid_properties.push('invalid value for "serial_number", the character length must be great than or equal to 7.')
      end
    ...
    def valid?
      return false if @approval_code.nil?
      return false if @approval_code.to_s.length > 6
      return false if @approval_code.to_s.length < 6
      return false if @serial_number.nil?
      return false if @serial_number.to_s.length > 15
      return false if @serial_number.to_s.length < 7
      true
    end
    ...
    def approval_code=(approval_code)
      if approval_code.nil?
        fail ArgumentError, 'approval_code cannot be nil'
      end
      ...
      if approval_code.to_s.length < 6
        fail ArgumentError, 'invalid value for "approval_code", the character length must be great than or equal to 6.'
      end
    ...
    def serial_number=(serial_number)
      ...
      if serial_number.to_s.length > 15
        fail ArgumentError, 'invalid value for "serial_number", the character length must be smaller than or equal to 15.'
      end

      if serial_number.to_s.length < 7
        fail ArgumentError, 'invalid value for "serial_number", the character length must be great than or equal to 7.'
      end
    ...
    def hash
      [approval_code, serial_number].hash
    end
