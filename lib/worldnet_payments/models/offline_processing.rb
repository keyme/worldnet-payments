...
module WorldnetPayments
  # Only use this field to indicate offline approvals or declines.
  class OfflineProcessing
    attr_accessor :operation

    # Approval code provided by the payment processor for the specific transaction.<br />Use this field combined with `OFFLINE_APPROVAL` operation.
    attr_accessor :approval_code

    # Local date/time of operation as per [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) standard.<br />Use this field combined with `DEFERRED_AUTHORIZATION` operation.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::OfflineProcessing` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::OfflineProcessing`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      operation_validator = EnumAttributeValidator.new('String', ["OFFLINE_DECLINE", "OFFLINE_APPROVAL", "DEFERRED_AUTHORIZATION"])
    ...
    def operation=(operation)
      validator = EnumAttributeValidator.new('String', ["OFFLINE_DECLINE", "OFFLINE_APPROVAL", "DEFERRED_AUTHORIZATION"])
    ...
    def hash
      [operation, approval_code, date_time].hash
    end
