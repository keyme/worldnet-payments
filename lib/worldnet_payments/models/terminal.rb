...
module WorldnetPayments
  class Terminal
    # The financial entity that will process and authorize the operations performed by the terminal.
    attr_accessor :payment_processor

    # Unique number assigned by the gateway.
    ...
    attr_accessor :terminal_features

    attr_accessor :fraud_detection

    # The list of rules that allows for changing the way transactions are processed by this terminal. Each rule has its own conditions which leads to a certain action when all of them are met.<br />For example, processing rules can be used to route transactions to other terminals in your account.
    attr_accessor :processing_rules

    # The list of terminal's pre-configured tips.
    attr_accessor :terminal_tips

    # The list of terminal's supported taxes.
    attr_accessor :terminal_taxes

    # List of additional data fields that can be used when performing transactions or storing secure credentials.<br />For more information about custom fields, see [Special Fields and Parameters](https://docs.worldnettps.com/doku.php?id=developer:api_specification:special_fields_and_parameters#the_custom_fields).
    attr_accessor :terminal_custom_fields

    # List of card brands supported by the terminal.
    attr_accessor :supported_cards

    # List of POS device types supported by the terminal.
    attr_accessor :supported_device_types
    ...
    def self.openapi_types
      {
        ...
        :'terminal_tips' => :'Array<TerminalTip>',
        :'terminal_taxes' => :'Array<TerminalTax>',
        :'terminal_custom_fields' => :'Array<TerminalCustomField>',
        :'supported_cards' => :'Array<String>',
        :'supported_device_types' => :'Array<String>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::Terminal` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::Terminal`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'terminal_number')
        self.terminal_number = attributes[:'terminal_number']
      end
      ...
      if attributes.key?(:'bank_settings')
        self.bank_settings = attributes[:'bank_settings']
      end
      ...
      if attributes.key?(:'terminal_features')
        self.terminal_features = attributes[:'terminal_features']
      end

      if attributes.key?(:'fraud_detection')
        self.fraud_detection = attributes[:'fraud_detection']
      end

      if attributes.key?(:'processing_rules')
        if (value = attributes[:'processing_rules']).is_a?(Array)
          self.processing_rules = value
        end
      end

      if attributes.key?(:'terminal_tips')
        if (value = attributes[:'terminal_tips']).is_a?(Array)
          self.terminal_tips = value
        end
      end

      if attributes.key?(:'terminal_taxes')
        if (value = attributes[:'terminal_taxes']).is_a?(Array)
          self.terminal_taxes = value
        end
      end

      if attributes.key?(:'terminal_custom_fields')
        if (value = attributes[:'terminal_custom_fields']).is_a?(Array)
          self.terminal_custom_fields = value
        end
      end

      if attributes.key?(:'supported_cards')
        if (value = attributes[:'supported_cards']).is_a?(Array)
          self.supported_cards = value
        end
      end

      if attributes.key?(:'supported_device_types')
        if (value = attributes[:'supported_device_types']).is_a?(Array)
          self.supported_device_types = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @payment_processor.nil?
        invalid_properties.push('invalid value for "payment_processor", payment_processor cannot be nil.')
      end

      if @payment_processor.to_s.length > 128
        invalid_properties.push('invalid value for "payment_processor", the character length must be smaller than or equal to 128.')
      end

      if @payment_processor.to_s.length < 0
        invalid_properties.push('invalid value for "payment_processor", the character length must be great than or equal to 0.')
      end

      if @terminal_number.nil?
        invalid_properties.push('invalid value for "terminal_number", terminal_number cannot be nil.')
      end

      if @terminal_number.to_s.length > 50
        invalid_properties.push('invalid value for "terminal_number", the character length must be smaller than or equal to 50.')
      end

      if @terminal_number.to_s.length < 1
        invalid_properties.push('invalid value for "terminal_number", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      return false if @payment_processor.nil?
      return false if @payment_processor.to_s.length > 128
      return false if @payment_processor.to_s.length < 0
      return false if @terminal_number.nil?
      return false if @terminal_number.to_s.length > 50
      return false if @terminal_number.to_s.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] payment_processor Value to be assigned
    def payment_processor=(payment_processor)
      if payment_processor.nil?
        fail ArgumentError, 'payment_processor cannot be nil'
      end

      if payment_processor.to_s.length > 128
        fail ArgumentError, 'invalid value for "payment_processor", the character length must be smaller than or equal to 128.'
      end

      if payment_processor.to_s.length < 0
        fail ArgumentError, 'invalid value for "payment_processor", the character length must be great than or equal to 0.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] terminal_number Value to be assigned
    def terminal_number=(terminal_number)
      if terminal_number.nil?
        fail ArgumentError, 'terminal_number cannot be nil'
      end

      if terminal_number.to_s.length > 50
        fail ArgumentError, 'invalid value for "terminal_number", the character length must be smaller than or equal to 50.'
      end

      if terminal_number.to_s.length < 1
        fail ArgumentError, 'invalid value for "terminal_number", the character length must be great than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] supported_cards Value to be assigned
    def supported_cards=(supported_cards)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          terminal_number == o.terminal_number &&
          merchant_details == o.merchant_details &&
          bank_settings == o.bank_settings &&
          terminal_location == o.terminal_location &&
          terminal_features == o.terminal_features &&
          fraud_detection == o.fraud_detection &&
          processing_rules == o.processing_rules &&
          terminal_tips == o.terminal_tips &&
          terminal_taxes == o.terminal_taxes &&
          terminal_custom_fields == o.terminal_custom_fields &&
          supported_cards == o.supported_cards &&
          supported_device_types == o.supported_device_types &&
    ...
    def hash
      [payment_processor, terminal_number, merchant_details, bank_settings, terminal_location, terminal_features, fraud_detection, processing_rules, terminal_tips, terminal_taxes, terminal_custom_fields, supported_cards, supported_device_types, volume_limits, links].hash
    end
