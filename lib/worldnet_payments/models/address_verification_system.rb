...
module WorldnetPayments
  class AddressVerificationSystem
    # Indicates whether AVS check is available for the terminal and merchants may send cardholder’s billing address for verification.
    attr_accessor :enable

    # If enabled, it ensures that a transaction will never be processed without billing address information.
    attr_accessor :compulsory

    # Indicates that all AVS non-match will lead to a transaction decline. This is sometimes desirable as not all card issuers will decline a transaction that has failed an AVS check.
    attr_accessor :auto_decline_on_failure

    # This field defines a configuration for the execution and validation of AVS transactions.  - Exact: At least the first line of the address as well as city and postal code are mandatory. - Postal: Only postal code is mandatory when filling up customer's billing address.
    attr_accessor :preferred_address_mode

    # List of AVS result codes that are considered to be a match. The codes left out of this list will lead to a transaction decline when `autoDeclineTransactionsOnAvsFailures` is enabled.
    attr_accessor :approval_codes
    ...
    def self.openapi_types
      {
        :'enable' => :'Boolean',
        :'compulsory' => :'Boolean',
        :'auto_decline_on_failure' => :'Boolean',
        :'preferred_address_mode' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::AddressVerificationSystem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::AddressVerificationSystem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'compulsory')
        self.compulsory = attributes[:'compulsory']
      end

      if attributes.key?(:'auto_decline_on_failure')
        self.auto_decline_on_failure = attributes[:'auto_decline_on_failure']
      end

      if attributes.key?(:'preferred_address_mode')
        self.preferred_address_mode = attributes[:'preferred_address_mode']
      end

      if attributes.key?(:'approval_codes')
        if (value = attributes[:'approval_codes']).is_a?(Array)
          self.approval_codes = value
        end
      end
    end
    ...
    def valid?
      preferred_address_mode_validator = EnumAttributeValidator.new('String', ["EXACT", "POSTAL"])
      return false unless preferred_address_mode_validator.valid?(@preferred_address_mode)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] preferred_address_mode Object to be assigned
    def preferred_address_mode=(preferred_address_mode)
      validator = EnumAttributeValidator.new('String', ["EXACT", "POSTAL"])
      unless validator.valid?(preferred_address_mode)
        fail ArgumentError, "invalid value for \"preferred_address_mode\", must be one of #{validator.allowable_values}."
      end
      @preferred_address_mode = preferred_address_mode
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          auto_decline_on_failure == o.auto_decline_on_failure &&
          preferred_address_mode == o.preferred_address_mode &&
          approval_codes == o.approval_codes
    end
    ...
    def hash
      [enable, compulsory, auto_decline_on_failure, preferred_address_mode, approval_codes].hash
    end
