...
module WorldnetPayments
  # Results of the security checks performed during authorization.
  class SecurityCheck
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::SecurityCheck` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::SecurityCheck`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      cvv_result_validator = EnumAttributeValidator.new('String', ["M", "N", "P", "U"])
      return false unless cvv_result_validator.valid?(@cvv_result)
      avs_result_validator = EnumAttributeValidator.new('String', ["Y", "A", "Z", "N", "U", "R", "G", "S", "F", "W", "X"])
      return false unless avs_result_validator.valid?(@avs_result)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] cvv_result Object to be assigned
    def cvv_result=(cvv_result)
      validator = EnumAttributeValidator.new('String', ["M", "N", "P", "U"])
      unless validator.valid?(cvv_result)
        fail ArgumentError, "invalid value for \"cvv_result\", must be one of #{validator.allowable_values}."
      end
      @cvv_result = cvv_result
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] avs_result Object to be assigned
    def avs_result=(avs_result)
      validator = EnumAttributeValidator.new('String', ["Y", "A", "Z", "N", "U", "R", "G", "S", "F", "W", "X"])
      unless validator.valid?(avs_result)
        fail ArgumentError, "invalid value for \"avs_result\", must be one of #{validator.allowable_values}."
      end
