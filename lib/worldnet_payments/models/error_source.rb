...
module WorldnetPayments
  class ErrorSource
    # The HTTP message element that contains the invalid value.
    attr_accessor :location

    # For errors in the request body, this field points to the object that contains the invalid property.
    attr_accessor :resource

    # The field or parameter that carries the invalid value.
    attr_accessor :property

    # Invalid value sent by the client.
    attr_accessor :value

    # Here the server gives a hint of how the client should fix the issue.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::ErrorSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::ErrorSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      location_validator = EnumAttributeValidator.new('String', ["HEADER", "BODY", "PATH", "QUERY"])
    ...
    def location=(location)
      validator = EnumAttributeValidator.new('String', ["HEADER", "BODY", "PATH", "QUERY"])
    ...
    def hash
      [location, resource, property, value, expected].hash
    end
