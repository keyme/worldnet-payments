  ...
  class UpdateTerminalRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::UpdateTerminalRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::UpdateTerminalRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] terminal_tips Value to be assigned
    def terminal_tips=(terminal_tips)
      @terminal_tips = terminal_tips
    end

    # Custom attribute writer method with validation
    # @param [Object] terminal_taxes Value to be assigned
    def terminal_taxes=(terminal_taxes)
    ...
    def hash
      [terminal_tips, terminal_taxes].hash
    end
