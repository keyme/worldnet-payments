  ...
  class PaymentRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::PaymentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::PaymentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'credential_on_file')
        self.credential_on_file = attributes[:'credential_on_file']
      end
      ...
      if attributes.key?(:'process_as_sale')
        self.process_as_sale = attributes[:'process_as_sale']
      else
        self.process_as_sale = false
      end

      if attributes.key?(:'offline_processing')
        self.offline_processing = attributes[:'offline_processing']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          credential_on_file == o.credential_on_file &&
          ip_address == o.ip_address &&
          auto_capture == o.auto_capture &&
          process_as_sale == o.process_as_sale &&
          offline_processing == o.offline_processing &&
    ...
    def hash
      [channel, terminal, operator, order, customer, customer_account, credential_on_file, ip_address, auto_capture, process_as_sale, offline_processing, additional_data_fields].hash
    end
