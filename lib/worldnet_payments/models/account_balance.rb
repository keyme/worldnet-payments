  ...
  class AccountBalance
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::AccountBalance` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::AccountBalance`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      benefit_category_validator = EnumAttributeValidator.new('String', ["CASH", "FOOD_STAMP"])
      return false unless benefit_category_validator.valid?(@benefit_category)
    ...
    # @param [Object] benefit_category Object to be assigned
    def benefit_category=(benefit_category)
      validator = EnumAttributeValidator.new('String', ["CASH", "FOOD_STAMP"])
      unless validator.valid?(benefit_category)
        fail ArgumentError, "invalid value for \"benefit_category\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [benefit_category, currency, amount].hash
    end
