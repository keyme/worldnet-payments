...
module WorldnetPayments
  ...
  class TerminalTip
    # Unique reference number assigned by the gateway. This field must be used when trying to update an existing terminal's tip.
    attr_accessor :unique_reference

    attr_accessor :type

    # This field must used when the tip is of type `FIXED_AMOUNT`. Otherwise, refer to the `percentage` property instead.
    attr_accessor :amount

    # Currency associated with the tip. A 3-letter code as per the [ISO 4217](https://en.wikipedia.org/wiki/ISO_4217#Active_codes) standard.<br />This is specially useful for multi-currency terminals in order to provide a different set of tips for each supported currency.
    attr_accessor :currency

    # This field must used when the tip is of type `PERCENTAGE`. Otherwise, refer to the `amount` property instead.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::TerminalTip` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::TerminalTip`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@unique_reference.nil? && @unique_reference.to_s.length > 10
      ...
      if !@percentage.nil? && @percentage <= 0
        invalid_properties.push('invalid value for "percentage", must be greater than 0.')
      end
    ...
    def valid?
      return false if !@unique_reference.nil? && @unique_reference.to_s.length > 10
      return false if !@unique_reference.nil? && @unique_reference.to_s.length < 10
    ...
    def percentage=(percentage)
      ...
      if !percentage.nil? && percentage <= 0
        fail ArgumentError, 'invalid value for "percentage", must be greater than 0.'
      end
    ...
    def hash
      [unique_reference, type, amount, currency, percentage].hash
    end
