...
module WorldnetPayments
  class SecureCredentialsPayloadAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::SecureCredentialsPayloadAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::SecureCredentialsPayloadAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@credentials_number.nil? && @credentials_number.to_s.length > 19
        invalid_properties.push('invalid value for "credentials_number", the character length must be smaller than or equal to 19.')
      end

      if !@credentials_number.nil? && @credentials_number.to_s.length < 12
    ...
    def valid?
      return false if !@credentials_number.nil? && @credentials_number.to_s.length > 19
      return false if !@credentials_number.nil? && @credentials_number.to_s.length < 12
      true
    end
