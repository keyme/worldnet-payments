...
module WorldnetPayments
  class ReversePaymentRequest
    ...
    # The amount to return to the customer's account.  **Note:** This field is not required for full amount cancellations.
    attr_accessor :reversal_amount

    # The reason why the transaction is being cancelled.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::ReversePaymentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::ReversePaymentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reversal_amount')
        self.reversal_amount = attributes[:'reversal_amount']
      end

      if attributes.key?(:'reversal_reason')
        self.reversal_reason = attributes[:'reversal_reason']
      end
    ...
    def list_invalid_properties
      ...
      if !@reversal_amount.nil? && @reversal_amount <= 0
        invalid_properties.push('invalid value for "reversal_amount", must be greater than 0.')
      end

      if !@reversal_reason.nil? && @reversal_reason.to_s.length > 100
        invalid_properties.push('invalid value for "reversal_reason", the character length must be smaller than or equal to 100.')
      end

      if !@reversal_reason.nil? && @reversal_reason.to_s.length < 1
        invalid_properties.push('invalid value for "reversal_reason", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@reversal_amount.nil? && @reversal_amount <= 0
      return false if !@reversal_reason.nil? && @reversal_reason.to_s.length > 100
      return false if !@reversal_reason.nil? && @reversal_reason.to_s.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] reversal_amount Value to be assigned
    def reversal_amount=(reversal_amount)
      if !reversal_amount.nil? && reversal_amount <= 0
        fail ArgumentError, 'invalid value for "reversal_amount", must be greater than 0.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] reversal_reason Value to be assigned
    def reversal_reason=(reversal_reason)
      if !reversal_reason.nil? && reversal_reason.to_s.length > 100
        fail ArgumentError, 'invalid value for "reversal_reason", the character length must be smaller than or equal to 100.'
      end

      if !reversal_reason.nil? && reversal_reason.to_s.length < 1
        fail ArgumentError, 'invalid value for "reversal_reason", the character length must be great than or equal to 1.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          operator == o.operator &&
          reversal_amount == o.reversal_amount &&
          reversal_reason == o.reversal_reason &&
    ...
    def hash
      [operator, reversal_amount, reversal_reason, customer_account, ip_address].hash
    end
