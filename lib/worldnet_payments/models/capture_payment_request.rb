...
module WorldnetPayments
  class CapturePaymentRequest
    ...
    # The amount to be captured. If not sent, the full amount of the transaction will be captured.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::CapturePaymentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::CapturePaymentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@capture_amount.nil? && @capture_amount <= 0
        invalid_properties.push('invalid value for "capture_amount", must be greater than 0.')
      end
    ...
    def valid?
      ...
      return false if !@capture_amount.nil? && @capture_amount <= 0
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] capture_amount Value to be assigned
    def capture_amount=(capture_amount)
      if !capture_amount.nil? && capture_amount <= 0
        fail ArgumentError, 'invalid value for "capture_amount", must be greater than 0.'
      end
