...
module WorldnetPayments
  class UpdateCredentialsRequest
    # The number of the terminal which is the credentials' holder.
    ...
    # List of custom fields to add or update. An empty list will lead to the removal of any existing additional data associated with the credentials.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::UpdateCredentialsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::UpdateCredentialsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [terminal, operator, customer, customer_account, additional_data_fields].hash
    end
