...
module WorldnetPayments
  class UpdatePaymentRequest
    ...
    # It allows clients to update the transaction's total amount.
    attr_accessor :total_amount

    attr_accessor :customer

    attr_accessor :order_breakdown

    # It allows clients to update the transaction's status.
    attr_accessor :status

    # It allows clients to perform a late signature capture for approved cardholder present transactions.  **Note:** Once a signature is stored for a specific transaction it cannot be replace or deleted.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::UpdatePaymentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::UpdatePaymentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'order_breakdown')
        self.order_breakdown = attributes[:'order_breakdown']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          customer == o.customer &&
          order_breakdown == o.order_breakdown &&
          status == o.status &&
    ...
    def hash
      [operator, total_amount, customer, order_breakdown, status, cardholder_signature].hash
    end
