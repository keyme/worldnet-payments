...
module WorldnetPayments
  class EmvPayload < Payload
    # This field gives clients the ability to reprocess offline transactions that were not approved initially in a different entry mode.<br />For instance, `EMV` transactions are allowed to be downgraded to `MAG_STRIPE` or `KEYED`.
    attr_accessor :downgrade_to

    attr_accessor :device

    # The EMV tags in TLV (Tag-Length-Value) format.<br />For clients not familiar with this format, the API also offer the possibility of sending the list of tags in the `tags` fields.
    attr_accessor :tlv

    # The list of EMV tags. Not required unless the `tlv` field is absent.
    attr_accessor :tags

    # The first digit of the primary account number (PAN) / card number.
    attr_accessor :first_digit_of_pan
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::EmvPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::EmvPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tlv')
        self.tlv = attributes[:'tlv']
      end
      ...
      if attributes.key?(:'first_digit_of_pan')
        self.first_digit_of_pan = attributes[:'first_digit_of_pan']
      end
    ...
    def list_invalid_properties
      ...
      if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length > 1
        invalid_properties.push('invalid value for "first_digit_of_pan", the character length must be smaller than or equal to 1.')
      end

      if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length < 1
        invalid_properties.push('invalid value for "first_digit_of_pan", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length > 1
      return false if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length < 1
      true && super
    end
    ...
    # @param [Object] first_digit_of_pan Value to be assigned
    def first_digit_of_pan=(first_digit_of_pan)
      if !first_digit_of_pan.nil? && first_digit_of_pan.to_s.length > 1
        fail ArgumentError, 'invalid value for "first_digit_of_pan", the character length must be smaller than or equal to 1.'
      end

      if !first_digit_of_pan.nil? && first_digit_of_pan.to_s.length < 1
        fail ArgumentError, 'invalid value for "first_digit_of_pan", the character length must be great than or equal to 1.'
      end

      @first_digit_of_pan = first_digit_of_pan
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          tlv == o.tlv &&
          tags == o.tags &&
          first_digit_of_pan == o.first_digit_of_pan &&
          cardholder_signature == o.cardholder_signature && super(o)
    end
    ...
    def hash
      [downgrade_to, device, tlv, tags, first_digit_of_pan, cardholder_signature].hash
    end
