...
module WorldnetPayments
  class EncryptedMagStripeDataFormat < MagStripeDataFormat
    attr_accessor :device

    # The encrypted data captured by the magnetic stripe reader.
    attr_accessor :encrypted_data

    # The first digit of the primary account number (PAN) / card number.
    attr_accessor :first_digit_of_pan

    # Indicates that the chip of the card could not be read due to a technical issue which resulted in the technology “falling back” to a magnetic stripe transaction.
    attr_accessor :fallback

    # The reason that caused the fallback.
    ...
    def self.openapi_all_of
      [
      :'EncryptedMagStripeDataFormatAllOf',
      :'MagStripeDataFormat'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::EncryptedMagStripeDataFormat` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::EncryptedMagStripeDataFormat`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @encrypted_data.to_s.length > 2147483647
        invalid_properties.push('invalid value for "encrypted_data", the character length must be smaller than or equal to 2147483647.')
      end

      if @encrypted_data.to_s.length < 1
        invalid_properties.push('invalid value for "encrypted_data", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @encrypted_data.to_s.length > 2147483647
      return false if @encrypted_data.to_s.length < 1
      return false if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length > 1
      return false if !@first_digit_of_pan.nil? && @first_digit_of_pan.to_s.length < 1
      fallback_reason_validator = EnumAttributeValidator.new('String', ["TECHNICAL", "EMPTY_CANDIDATE_LIST"])
      return false unless fallback_reason_validator.valid?(@fallback_reason)
      true && super
    end

    # Custom attribute writer method with validation
    # @param [Object] encrypted_data Value to be assigned
    def encrypted_data=(encrypted_data)
      if encrypted_data.nil?
        fail ArgumentError, 'encrypted_data cannot be nil'
      end

      if encrypted_data.to_s.length > 2147483647
        fail ArgumentError, 'invalid value for "encrypted_data", the character length must be smaller than or equal to 2147483647.'
      end

      if encrypted_data.to_s.length < 1
        fail ArgumentError, 'invalid value for "encrypted_data", the character length must be great than or equal to 1.'
      end
    ...
    def fallback_reason=(fallback_reason)
      validator = EnumAttributeValidator.new('String', ["TECHNICAL", "EMPTY_CANDIDATE_LIST"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          fallback == o.fallback &&
          fallback_reason == o.fallback_reason && super(o)
    end
    ...
    def hash
      [device, encrypted_data, first_digit_of_pan, fallback, fallback_reason].hash
    end
