...
module WorldnetPayments
  # The result details of the transaction between our platform and the payment processor.<br />For the full list of response codes, see [Transaction Responses](https://docs.worldnettps.com/doku.php?id=merchant:existing_merchant:other_information:transaction_responses).
  class TransactionResult
    ...
    # The amount authorized by the payment processor.  **Note:** For partial authorizations, this amount will be lower than the actual amount sent in the request.<br />**Note:** This amount will be negative for refund transactions to represent the return of funds back to the customer's account.
    ...
    # A brief description sent by the processor about the transaction result.<br />Clients must use this description in order to understand the cause of rejections and declines.
    attr_accessor :result_message

    # The original result code sent by the payment processor.
    attr_accessor :processor_result_code

    attr_accessor :stored_payment_credentials
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::TransactionResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::TransactionResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'authorized_amount')
      ...
      if attributes.key?(:'processor_result_code')
        self.processor_result_code = attributes[:'processor_result_code']
      end

      if attributes.key?(:'stored_payment_credentials')
        self.stored_payment_credentials = attributes[:'stored_payment_credentials']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@approval_code.nil? && @approval_code.to_s.length > 48
        invalid_properties.push('invalid value for "approval_code", the character length must be smaller than or equal to 48.')
      end

      if !@approval_code.nil? && @approval_code.to_s.length < 0
        invalid_properties.push('invalid value for "approval_code", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["SALE", "PREAUTH", "COMPLETION", "REFUND", "OFFLINE_DECLINE", "WITHDRAWAL"])
      ...
      return false if !@approval_code.nil? && @approval_code.to_s.length > 48
      return false if !@approval_code.nil? && @approval_code.to_s.length < 0
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["SALE", "PREAUTH", "COMPLETION", "REFUND", "OFFLINE_DECLINE", "WITHDRAWAL"])
    ...
    def approval_code=(approval_code)
      if !approval_code.nil? && approval_code.to_s.length > 48
        fail ArgumentError, 'invalid value for "approval_code", the character length must be smaller than or equal to 48.'
      end

      if !approval_code.nil? && approval_code.to_s.length < 0
        fail ArgumentError, 'invalid value for "approval_code", the character length must be great than or equal to 0.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          processor_result_code == o.processor_result_code &&
          stored_payment_credentials == o.stored_payment_credentials
    end
    ...
    def hash
      [type, status, approval_code, date_time, currency, authorized_amount, result_code, result_message, processor_result_code, stored_payment_credentials].hash
    end
