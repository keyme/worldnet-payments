...
module WorldnetPayments
  class EncryptionCapableDevice
    # Type/Model of the point-of-sale terminal.
    attr_accessor :type

    attr_accessor :data_ksn

    # Serial number of the point-of-sale terminal.
    attr_accessor :serial_number

    # Firmware version of the point-of-sale terminal.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::EncryptionCapableDevice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::EncryptionCapableDevice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'data_ksn')
        self.data_ksn = attributes[:'data_ksn']
      end
    ...
    def list_invalid_properties
      ...
      if @data_ksn.nil?
        invalid_properties.push('invalid value for "data_ksn", data_ksn cannot be nil.')
      end

      if @data_ksn.to_s.length > 20
        invalid_properties.push('invalid value for "data_ksn", the character length must be smaller than or equal to 20.')
      end

      if @data_ksn.to_s.length < 1
        invalid_properties.push('invalid value for "data_ksn", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @data_ksn.nil?
      return false if @data_ksn.to_s.length > 20
      return false if @data_ksn.to_s.length < 1
      category_validator = EnumAttributeValidator.new('String', ["ATTENDED", "UNATTENDED"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] data_ksn Value to be assigned
    def data_ksn=(data_ksn)
      if data_ksn.nil?
        fail ArgumentError, 'data_ksn cannot be nil'
      end

      if data_ksn.to_s.length > 20
        fail ArgumentError, 'invalid value for "data_ksn", the character length must be smaller than or equal to 20.'
      end

      if data_ksn.to_s.length < 1
        fail ArgumentError, 'invalid value for "data_ksn", the character length must be great than or equal to 1.'
      end
    ...
    def category=(category)
      validator = EnumAttributeValidator.new('String', ["ATTENDED", "UNATTENDED"])
    ...
    def hash
      [type, data_ksn, serial_number, firmware_version, category].hash
    end
