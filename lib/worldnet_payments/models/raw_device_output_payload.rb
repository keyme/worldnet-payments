...
module WorldnetPayments
  class RawDeviceOutputPayload < Payload
    attr_accessor :device

    # The raw POS device's output data.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::RawDeviceOutputPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::RawDeviceOutputPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @raw_data.to_s.length > 2147483647
        invalid_properties.push('invalid value for "raw_data", the character length must be smaller than or equal to 2147483647.')
      end

      if @raw_data.to_s.length < 1
        invalid_properties.push('invalid value for "raw_data", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @raw_data.to_s.length > 2147483647
      return false if @raw_data.to_s.length < 1
      true && super
    end
    ...
    def raw_data=(raw_data)
      if raw_data.nil?
        fail ArgumentError, 'raw_data cannot be nil'
      end

      if raw_data.to_s.length > 2147483647
        fail ArgumentError, 'invalid value for "raw_data", the character length must be smaller than or equal to 2147483647.'
      end

      if raw_data.to_s.length < 1
        fail ArgumentError, 'invalid value for "raw_data", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [device, raw_data, cardholder_signature].hash
    end
