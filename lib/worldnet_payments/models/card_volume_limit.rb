...
module WorldnetPayments
  # List of limits to be applied to transactions using the brand specified in the property `card`.
  class CardVolumeLimit
    attr_accessor :limit_scope

    # The maximum amount allowed within the period described in the `limitScope` attribute.
    attr_accessor :limit_threshold

    # If enabled, the limit will only take transactions without CVV into account. This is specially useful for merchants who are looking for a way to add thresholds for transactions that may lead to chargebacks.<br />This filter only applies to limits within `SINGLE_TRANSACTION` scope.
    attr_accessor :without_cvv_only

    # If enabled, the limit will only take transactions without AVS into account. This is specially useful for merchants who are looking for a way to add thresholds for transactions that may lead to chargebacks.<br />This filter only applies to limits within `SINGLE_TRANSACTION` scope.
    attr_accessor :without_avs_only

    # The sum of all transactions performed with the `card` within the period described in the `limitScope` attribute.<br />At the end of each period, this value is reset.
    ...
    def self.openapi_types
      {
        ...
        :'without_cvv_only' => :'Boolean',
        :'without_avs_only' => :'Boolean',
        :'current_value' => :'Float'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `WorldnetPayments::CardVolumeLimit` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `WorldnetPayments::CardVolumeLimit`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'limit_scope')
        self.limit_scope = attributes[:'limit_scope']
      end

      if attributes.key?(:'limit_threshold')
        self.limit_threshold = attributes[:'limit_threshold']
      end

      if attributes.key?(:'without_cvv_only')
        self.without_cvv_only = attributes[:'without_cvv_only']
      end

      if attributes.key?(:'without_avs_only')
        self.without_avs_only = attributes[:'without_avs_only']
      end
    ...
    def list_invalid_properties
      ...
      if @limit_scope.nil?
        invalid_properties.push('invalid value for "limit_scope", limit_scope cannot be nil.')
      end

      if @limit_threshold.nil?
        invalid_properties.push('invalid value for "limit_threshold", limit_threshold cannot be nil.')
      end
    ...
    def valid?
      return false if @limit_scope.nil?
      limit_scope_validator = EnumAttributeValidator.new('String', ["DAILY", "MONTHLY", "SINGLE_TRANSACTION"])
      return false unless limit_scope_validator.valid?(@limit_scope)
      return false if @limit_threshold.nil?
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] limit_scope Object to be assigned
    def limit_scope=(limit_scope)
      validator = EnumAttributeValidator.new('String', ["DAILY", "MONTHLY", "SINGLE_TRANSACTION"])
      unless validator.valid?(limit_scope)
        fail ArgumentError, "invalid value for \"limit_scope\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          limit_threshold == o.limit_threshold &&
          without_cvv_only == o.without_cvv_only &&
          without_avs_only == o.without_avs_only &&
    ...
    def hash
      [limit_scope, limit_threshold, without_cvv_only, without_avs_only, current_value].hash
    end
